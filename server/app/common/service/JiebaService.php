...
class JiebaService
{
     ...
     * @author fzr
     */
    public static function jiebaSplit(string $text): string
    {
        // 初始化分词
        self::initJieba();
        ...
        $processed = array_map(function ($token) {
            return preg_replace('/[\x{3000}-\x{303F}\x{FF00}-\x{FFEF}]/u', '', $token);
        }, $tokens);

        // 过滤空字符串和停用词
        $filtered = array_filter($processed, function ($item) {
            $trimmed = trim($item);
            return $trimmed !== '' && !in_array($trimmed, self::$stopWords);
        });

        return $filtered ? implode(' ', $filtered) : $text;
    }
     ...
     * @author fzr
     */
    public static function preprocessQuery(string $text, string $symbol = " | "): string
    {
        // 替换连续空格为 &
        $processed = preg_replace('/\s+/', ' & ', trim($text));
        // 移除非单词字符（保留字母、数字、下划线、空格、& 和中文字符）
