...
class RecallKnow
{
     ...
     * @author fzr
     */
    public static function embeddingRecall(string $model, string $query, array $knowIds, int $limit = 100): array
    {
        try {
            if (!$knowIds) {
                return [];
            ...
            $embeddingArr = $vectorService->toEmbedding($channel, $modelName, $query);
     ...
     * @author fzr
     */
    public static function mixedRecall(string $model, string $query, array $knowIds): array
    {
        // 语义检索
        $embeddingResults = self::embeddingRecall($model, $query, $knowIds);
        // 混合检索
