...
class KbChatService
{
    ...
    protected int $embSubId=0;             // 向量子模型ID
    ...
    protected array $historyContext = [];   // 历史上下文
    protected array $optimizeUsage = [];    // 问题优化消耗
    protected string $optimizeAlias = '';   // 问题优化别名
    protected int $optimizeSubId = 0;       // 问题优化模型
...
//        }
    ...
    private function getChatContext(): array
    {
        ...
        $this->historyContext = $chatRecords;
    ...
    {
        $searchMode = $this->robot['search_mode']?:'similar';
        $embModels = $this->embModelId . ':' . $this->embSubId;

        // 问题优化
        $questions = [$this->question];
        $questions = array_merge($questions, $this->askOptimize());

        // 发起检索
        $results = [];
        foreach ($questions as $query) {
            if ($searchMode != 'full') {
                $tokens = gpt_tokenizer_count($query);
                $strLength = $this->embUsage['str_length']??0;
                $totalTokens = $this->embUsage['total_tokens']??0;
                $this->embUsage = [
                    'prompt_tokens'      => $totalTokens + $tokens,
                    'total_tokens'       => $totalTokens + $tokens,
                    'completion_tokens'  => $totalTokens + $tokens,
                    'str_length'    => $strLength + mb_strlen($query)
                ];
            }

            if ($searchMode == 'similar') {
                $lists = RecallKnow::embeddingRecall($embModels, $query, $this->kbIds);
                $results[] = ['k'=>60, 'list'=>$lists];
            } elseif ($searchMode == 'full') {
                $lists = RecallKnow::fullTextRecall($query, $this->kbIds);
                $results[] = ['k'=>60, 'list'=>$lists];
            } elseif ($searchMode == 'mix') {
                $lists = RecallKnow::mixedRecall($embModels, $query, $this->kbIds);
                $results[] = ['k'=>60, 'list'=>$lists];
            }
        }
        ...
        // 结果处理(2): 重排模型
        $similar = $this->robot['search_similar'];
        $rankingStatus = boolval($this->robot['ranking_status']);
        if ($rankingStatus and $results) {
            $similar = $this->robot['ranking_score'];
            $rankingModel = $this->robot['ranking_model'];
            $results = (new RankerService($rankingModel))->sendAuto($this->question, $results, 0);
        }
        ...
        // 结果处理(4): 过滤最大Tokens
        $results = RecallUtils::filterMaxTokens($results, $this->robot['search_tokens']);
...
//    }
    ...
    private function saveChatRecord(): void
    {
        ...
        // 问题优化Tokens
        $optimizeUseTokens = 0;
        if ($this->optimizeUsage) {
            $optimizeUseTokens = tokens_price('chat', $this->optimizeSubId, $this->optimizeUsage['str_length']);
            $flowsUsage['flows'][] = [
                'name'              => 'optimize',
                'model'             => $this->optimizeAlias,
                'total_price'       => $optimizeUseTokens,
                'prompt_tokens'     => $this->optimizeUsage['prompt_tokens'],
                'completion_tokens' => $this->optimizeUsage['completion_tokens'],
                'total_tokens'      => $this->optimizeUsage['total_tokens'],
                'str_length'        => $this->optimizeUsage['str_length']
            ];
        }
        ...
        if (!$this->instruct and (!$this->chatVip || !$this->embVip)) {
            $changeAmount += $optimizeUseTokens;
            if (!$this->chatVip) {
     ...
     * @throws Exception
     */
    private function askOptimize(): array
    {
        if (!$this->robot['optimize_ask']) {
            return [];
        }

        if (!$this->robot['optimize_model']) {
            throw new Exception('请正确配置问题优化模型');
        }

        try {
            $models = Models::checkModels($this->robot['optimize_model']);
            $this->optimizeSubId = $models['model_sid'];
            $this->optimizeAlias = $models['alias'];
        } catch (Exception $e) {
            throw new Exception('问题优化模型: ' . $e->getMessage());
        }

        $history = "";
        foreach ($this->historyContext as $item) {
            $q = $item["ask"];
            $a = $item["reply"];
            if ($history) {
                $history .= "\n";
        ...
        try {
            $template = robotEnum::getPromptTpl('optimize');
            $template = str_replace('{{histories}}', $history, $template);
            $template = str_replace('{{query}}', $this->question, $template);
            $messages = [['role' => 'user', 'content' => $template]];

            $chatService = ChatService::AIChannelFactory($models['channel'], $models);
            $chatService->chatHttpRequest($messages);
            $content = $chatService->getReplyContent()[0];
            $this->optimizeUsage = $chatService->getUsage();
            ...
            throw new Exception('问题优化出错: '.$e->getMessage());
        }
    }
