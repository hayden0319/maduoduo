...
class VoiceLogic extends BaseLogic
{
    ...
    public static function generate(int $type, int $recordId, int $userId, bool $isShare = false): bool|array
    {
        try {
            ...
            //兼任科大讯飞旧版本发音人的问题
            $channelConfig['pronounce'] = '2';
            $defaultConfig = VoiceEnum::getInputChannelDefaultConfig(VoiceEnum::KDXF);
            if(VoiceEnum::KDXF == $voiceConfig['channel'] && !in_array($voiceConfig['configs']['kdxf']['pronounce'],array_keys($defaultConfig['pronounce_list']))){
                $channelConfig['pronounce'] = $defaultConfig['pronounce'];
            }
            // 语音合成对象
