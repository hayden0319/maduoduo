...
//            }
     ...
     * @author fzr
     */
    public static function tests(array $post): bool|array
    {
        try {
            $modelKbKnow = new KbKnow();
            ...
            $questions = [$question];
            if ($optimizeAsk) {
                $ask = self::askOptimize($question, $optimizeModel);
                $questions = array_merge($questions, $ask);
            }

            // 重排模型
            if ($rankingStatus) {
                $rank = (new ModelsCost())->where(['model_id'=>$rankingModel])->findOrEmpty()->toArray();
                if (!$rank) {
                    throw new Exception('重排模型不可用,请检查模型配置');
                }
                $rankingModel = $rank['model_id'].':'.$rank['id'];
            }
            ...
            // 结果处理(4): 过滤最大Tokens
            return RecallUtils::filterMaxTokens($results, $searchTokens);
        } catch (Exception $e) {
            self::setError($e->getMessage());
            return false;
        }
...
//    }
     ...
     * @author fzr
     */
    private static function askOptimize($question, $optimizeModel): array
    {
        try {
            $models = Models::checkModels($optimizeModel);
        } catch (Exception $e) {
            throw new Exception('问题优化模型: ' . $e->getMessage());
        }

        try {
            $prompt = ConfigService::get('robot_cfg', 'optimize_prompt1', '');
            $template = $prompt ?: RobotEnum::getPromptTpl('optimize');
            $template = str_replace('{{histories}}', '-', $template);
            $template = str_replace('{{query}}', $question, $template);
